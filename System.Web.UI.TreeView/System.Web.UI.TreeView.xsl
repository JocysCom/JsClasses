	...
	<xsl:template match="folder">
		...
		<table border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td>
					<a class="SWUITW_Folder">
						<xsl:if test="@code">
							<xsl:attribute name="onclick">
								System.Web.UI.TreeView.Toggle2(this, '<xsl:value-of select="@code"/>')
							</xsl:attribute>
						</xsl:if>
						<xsl:if test="not(@code)">
							<xsl:attribute name="onclick">System.Web.UI.TreeView.Toggle(this)</xsl:attribute>
						</xsl:if>
						<!-- If the treeview is unfold, the image minus (-) is displayed-->
						<xsl:if test="@expanded">
							<xsl:if test="@expanded='true'">
								<img src="{$param-img-directory}minus.png"/>
							</xsl:if>
							<!-- plus (+) otherwise-->
							<xsl:if test="@expanded='false'">
								<img src="{$param-img-directory}plus.png"/>
							</xsl:if>
						</xsl:if>
					...
					<a class="SWUITW_Leaf">
						...
						<xsl:choose>
							<xsl:when test="position()=last()">
								<img src="{$param-img-directory}lastlink.png"/>
							</xsl:when>
							<xsl:otherwise>
								<img src="{$param-img-directory}link.png"/>
							</xsl:otherwise>
						</xsl:choose>
						<img src="{$param-img-directory}{@img}">
							<!-- if the attribut alt is present-->
